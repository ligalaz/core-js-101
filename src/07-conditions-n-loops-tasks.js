...
function doRectanglesOverlap(rect1, rect2) {
  ...
  return !(r1.y < r2.y1 || r1.y1 > r2.y2 || r1.x1 < r2.x || r1.x > r2.x1);
}
...
function isInsideCircle(circle, point) {
  const coordX = (point.x - circle.center.x) ** 2;
  const coordY = (point.y - circle.center.y) ** 2;
...
function isCreditCardNumber(ccn) {
  let ccnArr = String(ccn).split('').reverse();
  ccnArr = ccnArr
    .map((item, idx) => {
      if (idx % 2 !== 0) {
...
function getDigitalRoot(num) {
  ...
  return String(sum) < 10 ? sum : getDigitalRoot(sum);
}
...
function evaluateTicTacToePosition(position) {
  const leftDiag = position[0][0];
  const rightDiag = position[0][2];

  if (leftDiag) {
    for (let i = 0; i < position.length; i += 1) {
      if (position[i][i] !== leftDiag) {
        break;
      }
      if (i === 2 && position[i][i] === leftDiag) {
        return leftDiag;
      }
    }
  }
  if (rightDiag) {
    for (let i = 0; i < position.length; i += 1) {
      if (position[i][2 - i] !== rightDiag) {
        break;
      }
      if (i === 2 && position[i][2 - i] === rightDiag) {
        return rightDiag;
      }
    }
  }
  ...
  for (let i = 0; i < position.length; i += 1) {
    const horizn = position[0][i];

    if (horizn) {
      for (let j = 1; j < position.length; j += 1) {
        if (position[j][i] !== horizn) {
          break;
        }
        if (j === 2 && position[j][i] === horizn) {
          return horizn;
        }
      }
    }
    const vertical = position[i][0];
    if (vertical) {
      for (let j = 1; j < position.length; j += 1) {
        if (position[i][j] !== vertical) {
          break;
        }
        if (j === 2 && position[i][j] === vertical) {
          return vertical;
        }
      }
    }
